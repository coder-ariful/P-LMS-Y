        ...
        <>
            <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
                ...
                <div className='max-w-xl z-10 text-gray-500'>
                    <h1 className='md:text-course-details-heading-larger text-course-details-heading-small font-semibold text-gray-800'>{courseData.courseTitle}</h1>
                    <p className='pt-4 md:text-base text-sm' dangerouslySetInnerHTML={{ __html: courseData.courseDescription.slice(0, 200) }}></p>

                    <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                        <p>{calculateRating(courseData)}</p>
                        <div className='flex'>
                            {[...Array(5)].map((_, i) => (<img key={i} src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt='star image' className='w-3.5 h-3.5' />))}
                        </div>
                    ...
                    <p className='text-sm'>Course by <span className='text-blue-600 underline'>{courseData.teacherName}</span></p>
                                ...
                                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                                    <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={() => toggleSection(index)}>
                                        <div className='flex items-center gap-2'>
                                            <img src={assets.down_arrow_icon} alt="down arrow icon" className={`transform transition-transform ${openSections[index] ? 'rotate-180' : ''} `} />
                                            <p className='font-medium md:text-base text-sm'>{chapter.chapterTitle}</p>
                                        </div>
                                        <p className='text-sm md:text-[16px]'>{chapter.chapterContent.length} lectures - {calculateChapterTime(chapter)}</p>
                                    </div>
                ...
                <div className='max-w-[424px] z-1 shadow rounded-t-2xl md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
                    ...
                    <div className='p-5'>
                        ...
                        <div className='flex items-center text-sm md:text-[16px] gap-4 pt-2 md:pt-4 text-gray-500'>
                            ...
                            <div className='flex items-center gap-1 '>
                                <img src={assets.time_clock_icon} alt="clock icon or image" />
                        ...
                        <div className='pt-6'>
                            ...
                            <ul className='ml-4 pt-2 text-sm md:text-[16px] list-disc text-gray-500'>
                                ...
                                <li>Quizzes to your knowledge.</li>
